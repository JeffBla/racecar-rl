...
class World(world.World):
    ...
    def init(self) -> None:
        ...
        # Camera settings
        camera_distance = 19.2
        camera_pitch = -62.6
        camera_yaw = -213.6
        cam_target_pos = [4.89, -9.3, -3.42]
        p.resetDebugVisualizerCamera(camera_distance, camera_yaw, camera_pitch, cam_target_pos)
    ...
    def _update_race_info(self, agent):
        ...
        obstacle_map = self._maps['obstacle']
        dist_goal_map = self._maps['dist_goal']
        ...
        self._state[agent.id]['dist_goal'] = dist_goal
    ...
    def render(self, agent_id: str, mode: str, width=640, height=480, **kwargs) -> np.ndarray:
        ...
        elif mode == 'higher_birds_eye':
            return util.higher_birds_eye(agent=agent, width=width, height=height, **kwargs)
